...
def get_runtime_paths():
    ...
    return {
        "acc_data": resolve_resource_path(cfg["Data_acc"]["data_file"]),
        "gyr_data": resolve_resource_path(cfg["Data_gyr"]["data_file"]),
        "gyr_test": resolve_resource_path("Gyr_test.csv"),
        "model_acc": resolve_resource_path(cfg["model"]["model_acc"]),
        "model_gyr": resolve_resource_path(cfg["model"]["model_gyr"]),
    }


def get_missing_required_files():
    runtime_paths = get_runtime_paths()
    missing_paths = [path for path in runtime_paths.values() if not os.path.exists(path)]
...
def data_reshape_acc(X, y, TIME_STEP_acc, n_features_acc):
    ...
    # SciPy>=1.11 changed mode return behavior; keepdims keeps consistent indexing.
    label = stats.mode(y, keepdims=True).mode[0]
...
def preprocess_file_gyr(data_path_gyr, data_path_gyr_test):
    ...
    X_test = pd.read_csv(data_path_gyr_test)
...
def main():
    ...
    missing_paths = get_missing_required_files()
    if missing_paths:
        missing_str = ", ".join(missing_paths)
        APP_ERROR = f"Required files are missing: {missing_str}"
        return render_template('index.html', error_message=APP_ERROR)

    runtime_paths = get_runtime_paths()

    try:
        X_acc, y_acc = preprocess_file_acc(runtime_paths["acc_data"])
        X_test_gyro, X_gyr, y_gyr = preprocess_file_gyr(runtime_paths["gyr_data"], runtime_paths["gyr_test"])
        APP_ERROR = None
    except Exception as exc:
        APP_ERROR = f"Failed to initialize data: {exc}"
        return render_template('index.html', error_message=APP_ERROR)
...
def data():
    ...
    if APP_ERROR:
        response = make_response(json.dumps([[], APP_ERROR, APP_ERROR, [], []]))
        response.content_type = 'application/json'
        return response

    if "X_acc" not in globals() or "X_gyr" not in globals() or "y_acc" not in globals() or "y_gyr" not in globals() or "X_test_gyro" not in globals():
        response = make_response(json.dumps([[], "Initialization pending. Open '/' first.", "Initialization pending. Open '/' first.", [], []]))
        response.content_type = 'application/json'
        return response
    ...
    try:
        model_acc = load_model(runtime_paths["model_acc"])
        model_gyr = joblib.load(runtime_paths["model_gyr"])
    except Exception as exc:
        APP_ERROR = f"Failed to load model files: {exc}"
        response = make_response(json.dumps([[], APP_ERROR, APP_ERROR, [], []]))
        response.content_type = 'application/json'
        return response
